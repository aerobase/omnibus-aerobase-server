...
name "unifiedpush-server-package-scripts"
...
# according to git.
default_version `git ls-tree HEAD -- config/templates/unifiedpush-server-package-scripts | awk '{ print $3 }'`
...
build do
  ...
  Dir.glob(File.join(Omnibus::Config.project_root, 'config/templates/unifiedpush-server-package-scripts/*.erb')).each do |package_script|
